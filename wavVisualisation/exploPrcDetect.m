...
% Exploration vector
file2Detect = ['Z:\DATA\missions\2021-07-27_IML_2021-016_BRings\results\Detections_manuelles\DETECT_NARW_PRC_PR_V1.mat'];
%file2Detect = ['/Users/Administrator/Documents/MPO/BRing/Data/wav/' 'DETECT_NARW_PRC_PR_V1.mat'] ;
D = load(file2Detect);
nbF = numel(D.dtime);
...
%%
for ii=1:nbF
disp( ['Exploring file' getWavName(D.dtime(ii),folderIn) ] ) 
    
% Selected time
ptime = D.dtime(ii);
[wavname, wavi] = getWavName(ptime, folderIn);
...
angleMD(ii) = angleM;
angleAD(ii,:) = angleA;
energiedB(ii) = Energie_dB(pkloc(1));
file{ii} = fileList{1};
end % Of big loop

save(file2Detect,'angleM','angleA','energiedB','file','-append')
disp('Done !')
