...
outName = 'pass1';%'OffSet23_voc3_Ns2e14_150-200hz_AvCircle';
...
[ploc, ptime ]  = getPingLoc('aav'); % Load ping information
...
% Figure parameters
showFig = [ 2 3]; %7 9]%[1 2 3 4 7];% [1:7];% Figure number to print
...
offset = - (90-67) * pi /180; %(90 + 25) * pi / 180;  % Av = (90-67) CD  =
...
% the first or the last need to be change
if angleA(end) < 270
    angleA = [angleA(end) angleA(1:end-1)];
    pNb = [pNb(end)  pNb(1:end-1)];
end

% Linéaire regressino
x = 1:10;
p = polyfit(x,angleA,1);
...
ax = gca;
ax.XTickLabel = pNb;
leg= legend([hA,hTh,hFit],{'Ping','Theoritical curve','Lin. Reg. Curve'},'Location','northwest')
