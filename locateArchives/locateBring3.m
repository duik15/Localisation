...
% array.y
% Version 3 : Save of orignial version cos and sin 
clear all
close all

disp('Wrong version! Original script.')
% Path information
addpath(genpath('../cedBeamForming/'))   % Cedric tools
folderIn = '~/Documents/MPO/BRing/Data/wav/AV_circle/';
%folderIn = 'Z:\DATA\missions\2021-07-27_IML_2021-016_BRings\wav\';
outName = 'firstversion_AvCircle';
folderOut = ['/Users/Administrator/Documents/MPO/BRing/Data/results/Av/' outName '/'];

% Loading files and time
% Or give an array of time to process ptime = [datetime(xxx),datetime(xxx)]
[ploc, ptime ]  = getPingLoc('cav'); % Load ping information
...
offset = 0 %(90 + 25) * pi / 180;  % Av = 25deg
xc = R*cos(theta +  offset);
yc = R*sin(theta +  offset);
...
for iFile = 1:length(fileList)
    ...
    [MAT_s,fe, tstart, dura] = bringRead([folderIn fileList{iFile}], ptime(iFile),'duration',dura,'buffer',buffer,'power2',true);
...
%% Saving data
save([folderOut 'locateBRING_Data_' outName '.mat'],'aMax','matEnergie')
